...
"),
    ...
    'compress_gzip'=>('cat {{file}} | gzip > file.gz'),
...
",
    ...
    'mysql_restore'=>'mysql -u user --p={{password}} {{dbname}} [$tablemane] < {{backup_db_file}}',
    'mysql_explain'=>'EXPLAIN SELECT * FROM table where x=1 ',
    'mysql_import'=>'
        DROP TABLE IF EXISTS `$dbname`.`$table_name`;
        CREATE TABLE `$dbname`.`$table_name` LIKE `$dbname_source`.`$table_name`;
        INSERT INTO `$dbname`.`$table_name` SELECT * FROM `$dbname_source`.`$table_name`;
    ',
    ...
    'mysql_adminer'=>'
    cd /var/www/html
    sudo wget "http://www.adminer.org/latest.php" -O /var/www/html/adminer_$version.php
    ...
    # put into the file
    sudo nano adminer_$version.php
    ...
    // network commands
    //----------------------------------------------------------------------------
    'scp_upload'=>('sshpass -p "$pass" scp -o  StrictHostKeyChecking=no $l_path tmirandola@$host:$r_path'),
    'scp_download'=>('sshpass -p "$pass" scp -o  StrictHostKeyChecking=no tmirandola@$host:$r_path $l_path'),
    'ssh'=>'
